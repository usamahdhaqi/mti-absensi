...
if (isset($_POST['submit'])) {
    ...
    $sql_cek_karyawan = "SELECT nama_siswa FROM siswa WHERE nis = '$nis_input'";
    $hasil_cek_karyawan = mysqli_query($con, $sql_cek_karyawan);
    
    if (mysqli_num_rows($hasil_cek_karyawan) > 0) {
        $data_siswa = mysqli_fetch_assoc($hasil_cek_karyawan);
        ...
        if (move_uploaded_file($lampiran['tmp_name'], $path_tujuan)) {
            ...
            $sql_insert = "INSERT INTO ijin_siswa (
                                nama_siswa, ijin, alasan_ijin, tanggal_ijin, waktu_buat_ijin, 
                                atasan, lampiran, tipe_lampiran, 
                                aktif_notif_manager, aktif_appdpp_manager, app, 
                                app_by, alasan_app_dpp, aktif_notif_karyawan
                           ) 
                           VALUES (
                                '$nama_siswa', '$db_ijin_tipe', '$db_ijin_alasan', '$tanggal_ijin', '$db_waktu_buat', 
            ...
            if (mysqli_query($con, $sql_insert)) {
                tampilkan_pesan(
                    'sukses', // (Hijau)
                    'Pengajuan Terkirim!',
                    "Terima kasih, $nama_siswa. Pengajuan izin Anda telah terkirim.",
        ...
        tampilkan_pesan(
            'error', // (Merah)
            'ID Tidak Ditemukan',
            "ID siswa '$nis_input' tidak ditemukan atau tidak terdaftar.",
