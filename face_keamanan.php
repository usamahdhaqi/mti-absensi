...
<head>
  ...
  <title>MTI Absensi | Data Keamanan</title>
 ...
 if(isset($_POST['btn-submit'])){
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <h1>
        Data Keamanan
        <small>Histori Pelanggaran Keamanan</small>
      </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      ...
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title"><i class="fa fa-filter"></i> Filter Data Keamanan</h3>
        </div>
        <div class="box-body">
          <form action="face_keamanan.php" method="post">
            <div class="row">
              <div class="col-md-3">
        ...
        <div class="box-header with-border">
          <h3 class="box-title"><i class="fa fa-table"></i> Data Pelanggaran Keamanan</h3>
          ...
          <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover">
                  ...
                  <?php
                   ...
                   // Base query
                   $base_sql = "FROM `face_keamanan` JOIN employee ON face_keamanan.nama=employee.nama_pegawai WHERE status='melanggar' ";
                   ...
                   $sqlemp .= "ORDER BY face_keamanan.waktu DESC ";
                   $sqlemp .= "LIMIT $offset, $no_of_records_per_page";
                   
                   $query = $con->query($sqlemp);
                   $no=1; // ID Modal
                   $noe=$offset+1; // Nomor baris
                   while ($row = $query->fetch_assoc()) {
                            ...
                            echo '<td><span class="label label-danger">'. htmlspecialchars($row['status']) . '</span></td>';
                            
                            $foto_pelanggaran = 'hasil_keamanan/'. $row['nama'] . $row['waktu'] . '.jpg';
                            echo '<td>';
                            if (file_exists($foto_pelanggaran)) {
                              echo '<img style="width:80px;height:auto; cursor:pointer;" id="'.$no++.'" data-toggle="modal" data-target="#myModal" src="'.$foto_pelanggaran.'" alt="'. htmlspecialchars($row['nama']) . '" />';
                            } else {
                              echo 'N/A';
                            }
                            echo '</td>';
